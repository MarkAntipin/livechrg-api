...
# apply urls to all tests in the class
class TestClass:
    async def test_add_stations__station_does_not_exists(self, client: TestClient, pg: asyncpg.Pool, url: str) -> None:
        # act
        resp = client.post(
            url,
            json={
    ...
    async def test_add_stations__station_with_source_exists(
            self, client: TestClient, pg: asyncpg.Pool, url: str
    ) -> None:
        # arrange
        source = 'plug_share'
        station_inner_id = 1
